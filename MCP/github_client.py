import requests

from MCP import config


def _auth_headers(extra: dict = None) -> dict:
    """Build request headers, raising early if a token-requiring call has no token."""
    if not config.GITHUB_TOKEN:
        raise RuntimeError(
            "GITHUB_TOKEN is not set. Add it to your .env file before calling "
            "write/delete endpoints."
        )
    headers = dict(config.HEADERS)
    if extra:
        headers.update(extra)
    return headers


def _request(method: str, endpoint: str, headers: dict, params=None, json=None):
    """Perform a single HTTP request against the GitHub API. No retry/backoff."""
    url = f"{config.GITHUB_API}{endpoint}"
    try:
        response = requests.request(
            method,
            url,
            headers=headers,
            params=params,
            json=json,
            timeout=15,
        )
    except requests.exceptions.RequestException as e:
        raise RuntimeError(f"Network error calling GitHub API: {e}") from e

    return response


def github_get(endpoint: str, params=None):
    response = _request("GET", endpoint, headers=config.HEADERS, params=params)

    if response.status_code == 404:
        raise ValueError("Resource not found.")

    if not response.ok:
        raise RuntimeError(f"GitHub API Error {response.status_code}: {response.text}")

    return response.json()


def github_put(endpoint: str, json: dict):
    """Send PUT request to GitHub API."""
    headers = _auth_headers()
    response = _request("PUT", endpoint, headers=headers, json=json)

    if response.status_code == 404:
        raise ValueError("Resource not found.")

    if response.status_code not in (200, 201):
        raise RuntimeError(f"GitHub API Error {response.status_code}: {response.text}")

    return response.json()


def github_post(endpoint: str, json: dict):
    """Send POST request to GitHub API."""
    headers = _auth_headers()
    response = _request("POST", endpoint, headers=headers, json=json)

    if response.status_code == 404:
        raise ValueError("Resource not found.")

    if response.status_code not in (200, 201):
        raise RuntimeError(f"GitHub API Error {response.status_code}: {response.text}")

    return response.json()


def git_delete(endpoint: str, json: dict = None):
    headers = _auth_headers()
    response = _request("DELETE", endpoint, headers=headers, json=json)

    if response.status_code == 404:
        raise ValueError("Resource not found.")

    if response.status_code not in (200, 204):
        raise RuntimeError(f"GitHub API Error {response.status_code}: {response.text}")

    if response.status_code == 204 or not response.content:
        return {"status": "success"}

    return response.json()


def github_patch(endpoint: str, payload: dict):
    headers = _auth_headers()
    response = _request("PATCH", endpoint, headers=headers, json=payload)

    if response.status_code == 404:
        raise ValueError("Resource not found.")

    if response.status_code not in (200, 201):
        raise RuntimeError(f"GitHub API Error {response.status_code}: {response.text}")

    return response.json()
